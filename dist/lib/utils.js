...
export const shareLine = function (url) {
    window.open(`http://line.naver.jp/R/msg/text/?${url}`);
};
export const shareTwitter = function (url) {
    window.open(`https://twitter.com/intent/tweet?url=${url}`);
};
export const shareLinkedin = function (url) {
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${url}`);
};
export const numberFormat = function (num, options = {}) {
...
export const numberWithKMB = function (num, args) {
    const si = args.si || [
        { value: 1, symbol: '' },
        { value: 1E3, symbol: 'K' },
        { value: 1E6, symbol: 'M' },
...
export const roundDecimal = function (val, digits = 0) {
...
// array chunk
export const arrayChunk = function (myArray, chunkSize) {
    let index = 0;
...
// 滾動到特定 el
export const scrollToSection = function ({ el, offset = 0, jump = false }, callback) {
    if (el) {
    ...
    if (el) {
        switch (based) {
            case 'top':
                ...
                break;
            case 'bottom':
                const elBottomPosition = el.offsetTop - el.offsetHeight - window.pageYOffset;
...
// 複製文字
export async function copyText(passedString) {
    try {
...
// 數字前面補 0
export const padLeft = function (n, width, z = '0') {
    z = z || '0';
    n = n + '';
